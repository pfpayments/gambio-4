<?php declare(strict_types=1);
	...
	class PostFinanceCheckoutOrderActionController extends AdminHttpViewController
	{
		...
		/**
		 * @var PostFinanceCheckoutTransactionModel $transactionModel
		 */
		...
		public function __construct(HttpContextReaderInterface $httpContextReader, HttpResponseProcessorInterface $httpResponseProcessor, ContentViewInterface $defaultContentView)
		{
			...
			$this->transactionModel = new PostFinanceCheckoutTransactionModel();
		...
		public function actionDownloadFile()
		{
			...
			$transactionStateFulfill = PostFinanceCheckoutTransactionModel::TRANSACTION_STATE_FULFILL;
			
			$allowedStates = [
				$transactionStateFulfill,
				PostFinanceCheckoutTransactionModel::TRANSACTION_STATE_REFUNDED,
				PostFinanceCheckoutTransactionModel::TRANSACTION_STATE_PARTIALLY_REFUNDED,
				PostFinanceCheckoutTransactionModel::TRANSACTION_STATE_PAID
			];
		...
		public function actionRefund()
		{
			...
			$alreadyRefundedAmount = PostFinanceCheckoutRefundModel::getTotalRefundsAmount(
				PostFinanceCheckoutRefundModel::getRefunds($orderId)
			);
			...
			$transactionStatePaid = PostFinanceCheckoutTransactionModel::TRANSACTION_STATE_PAID;
			if (
		...
		protected function getUnauthorizedResponse(): ?HttpControllerResponse
		{
			...
			if ($this->isAuthenticatedAdmin() && PostFinanceCheckoutPageToken::isValid($pageToken)) {
				return null;
			}
