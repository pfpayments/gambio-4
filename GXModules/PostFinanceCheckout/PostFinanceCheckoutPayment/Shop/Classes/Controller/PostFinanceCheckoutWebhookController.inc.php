...
use GXModules\PostFinanceCheckoutPayment\Library\Core\{
...
class PostFinanceCheckoutWebhookController extends HttpViewController
{
	/**
	...
	public function actionIndex()
	{
		...
		switch ($listenerEntityTechnicalName) {
			...
			case WebHookRequest::REFUND:
				...
				PostFinanceCheckoutRefundModel::createRefundRecord((int)$data['entityId'], $orderId, $refund->getAmount());
				$refunds = PostFinanceCheckoutRefundModel::getRefunds($orderId);
				$amountToBeRefunded = floatval($transactionInfo['info']['total']) - PostFinanceCheckoutRefundModel::getTotalRefundsAmount($refunds);
