...
use GXModules\PostFinanceCheckout\PostFinanceCheckoutPayment\Shop\Classes\Entity\PostFinanceCheckoutTransactionEntity;

class PostFinanceCheckoutTransactionModel
{
	...
	public function getByOrderId(int $orderId): ?PostFinanceCheckoutTransactionEntity
	{
		...
		return new PostFinanceCheckoutTransactionEntity($orderData);
	}
	...
	public function create(Settings $settings, string $transactionId, string $orderId, array $orderData): void
	{
		...
		xtc_db_perform('postfinancecheckout_transactions', $insertData, 'insert');
	}
	...
	private function getFromDbByOrderId(int $orderId): ?array {
		$query = xtc_db_query("SELECT * FROM `postfinancecheckout_transactions` WHERE order_id = " . xtc_db_input($orderId));
