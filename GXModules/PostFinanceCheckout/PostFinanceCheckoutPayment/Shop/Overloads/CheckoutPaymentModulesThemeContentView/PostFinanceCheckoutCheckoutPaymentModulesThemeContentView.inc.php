...
use JTL\Shop;
use PostFinanceCheckout\Sdk\Model\{AddressCreate, LineItemCreate, LineItemType, Transaction, TransactionPending};

class PostFinanceCheckoutCheckoutPaymentModulesThemeContentView extends PostFinanceCheckoutCheckoutPaymentModulesThemeContentView_parent
{
    ...
    public function __construct()
    {
        ...
        $this->set_content_template('GXModules/PostFinanceCheckout/PostFinanceCheckoutPayment/Shop/Themes/All/checkout_payment_modules.html');
        $this->set_flat_assigns(true);
    }

    public function prepare_data()
    {
        $key = 'configuration/MODULE_PAYMENT_POSTFINANCECHECKOUT_STATUS';
            ...
            if ($order->info['total'] > 0) {
                ...
                if (!$arrayOfPossibleMethods) {
                    ...
                    foreach ($possiblePaymentMethods as $possiblePaymentMethod) {
                        $arrayOfPossibleMethods[] = 'postfinancecheckout_' . PostFinanceCheckoutHelper::slugify($possiblePaymentMethod->getName(), '_');
                    }
                ...
                $paymentMethodConfKey = 'configuration/MODULE_PAYMENT_POSTFINANCECHECKOUT_';
                ...
                $array = [];
                foreach ($selection as $t_key => $t_method_array) {
                    if (strpos(strtolower($t_method_array['id']), 'postfinancecheckout') !== false) {
                        if (strtolower($t_method_array['id']) !== 'postfinancecheckout') {
