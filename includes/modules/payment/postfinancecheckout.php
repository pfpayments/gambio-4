...
if (file_exists(dirname(__DIR__) . '/../../../GXModules/postfinancecheckout/PostFinanceCheckoutPayment/vendor/autoload.php')) {
    require_once dirname(__DIR__) . '/../../../GXModules/postfinancecheckout/PostFinanceCheckoutPayment/vendor/autoload.php';
}

defined('GM_HTTP_SERVER') || define('GM_HTTP_SERVER', HTTP_SERVER);

/**
 * Class postfinancecheckout_ORIGIN
 */
class postfinancecheckout_ORIGIN
{
    ...
    public function confirmation()
    {
        ...
        foreach ($_SESSION['possiblePaymentMethods'] as $paymentMethod) {
            $slug = 'postfinancecheckout_' . trim(strtolower(PostFinanceCheckoutHelper::slugify($paymentMethod->getName())));
    ...
    {
        $redirectUrl = xtc_href_link('shop.php', 'do=PostFinanceCheckoutPayment/PaymentPage?payment_error=' . $this->code, 'SSL');
    ...
    {
        if (isset($_SESSION['postfinancecheckout_error'])) {
            $error = array('error' => $_SESSION['postfinancecheckout_error']);
            unset($_SESSION['postfinancecheckout_error']);
            return $error;
        }
        return false;
    }
    ...
    public function install()
    {
        ...
        $defaultOrderStatus = [
            'TMPORDER_STATUS_ID' => [
                'names' => ['en' => 'postfinancecheckout temporary', 'de' => 'postfinancecheckout temporaer'],
                'color' => '2196F3',
            ],
            'ORDER_STATUS_ID' => [
    ...
    public function _configuration()
    {
        ...
        foreach ($this->getPaymentMethods() as $method) {
            ...
            define('MODULE_PAYMENT_POSTFINANCECHECKOUT_' . strtoupper($method['id']) . '_TITLE', $title);
            define('MODULE_PAYMENT_POSTFINANCECHECKOUT_' . strtoupper($method['id']) . '_DESC', $this->languageTextManager->get_text('would_you_like_to_enable_this_payment_method', 'postfinancecheckout'));
    ...
    {
        $configuration = \MainFactory::create('PostFinanceCheckoutStorage');
    ...
    protected function _initLanguageConstants()
    {
        ...
        foreach ($constantNames as $constantName) {
            ...
            defined($constantName) or define($constantName, $this->languageTextManager->get_text($translationKey, 'postfinancecheckout'));
        }
    }

}

MainFactory::load_origin_class('postfinancecheckout');
